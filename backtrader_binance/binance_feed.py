...
class BinanceData(DataBase):
    ...
    def _parser_to_kline(self, timestamp, kline):
        df = pd.DataFrame(
            [[timestamp, kline["o"], kline["h"], kline["l"], kline["c"], kline["v"]]]
        )
    ...
    def _start_live(self):
        ...
        self._store.binance_socket.start_kline_socket(
            self._handle_kline_socket_message, self.symbol_info["symbol"], self.interval
        )
    ...
    def start(self):
        ...
        if self.start_date:
            ...
            df.drop(
                df.columns[[6, 7, 8, 9, 10, 11]], axis=1, inplace=True
